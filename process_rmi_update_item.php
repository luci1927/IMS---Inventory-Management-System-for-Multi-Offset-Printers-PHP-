...
$inventory_rs = Database::search("SELECT * FROM `rmi_inventory` WHERE `item_code`='" . $item_code . "'");
if ($inventory_rs) {
...
if (empty($qhand)) {
    ...
    $stock_rs = Database::search("SELECT * FROM `rmi_stock` 
WHERE `rmi_inventory_item_code` = '" . $item_code . "' ORDER BY `date_time` DESC LIMIT 1;");
    ...
    Database::iud("INSERT INTO `rmi_stock` 
    (`rmi_inventory_item_code`,`qty_system`,`qty_hand`,`remarks`,`date_time`)
